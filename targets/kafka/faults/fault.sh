#!/usr/bin/env bash
#
# fault.sh — inject and heal faults on a Kafka cluster node for the
...
extra: isr-shrink, leader-election are recognized but unsupported (stubs).
...
do_kill() {
    case "$METHOD" in
        ...
        ssh)    sexec "sudo pkill -9 -f '$(param proc kafka)'" ;;
        *) return 3 ;;
    esac
}
do_pause() {
    case "$METHOD" in
        ...
        ssh)    sexec "sudo pkill -STOP -f '$(param proc kafka)'" ;;
        *) return 3 ;;
    esac
}
do_partition() {
...
# target_inject handles $TARGET-specific fault types. isr-shrink and
# leader-election are recognized but not yet implemented: they are stubs that
# report unsupported and exit non-zero, so a schedule referencing them fails
# loudly rather than silently doing nothing.
target_inject() {
    case "$TYPE" in
        isr-shrink|leader-election)
            echo "$PROG: fault type '$TYPE' is unsupported (stub) for $TARGET" >&2
            return 3
            ;;
        *)
...
heal() {
    case "$METHOD" in
        ...
        k8s)
            ...
            ;;
        ssh)
            sexec "sudo pkill -CONT -f '$(param proc kafka)'" 2>/dev/null || true
