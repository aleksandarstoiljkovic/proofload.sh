...
// dsnEnv is the environment variable that enables the integration tests. When it
// is unset the tests skip, so the default `go test ./...` needs no database.
const dsnEnv = "PROOFLOAD_PG_DSN"

// integrationConfig returns a driver.Config pointed at the DSN from the
...
func integrationConfig(t *testing.T) driver.Config {
	...
	return driver.Config{Endpoints: []string{dsn}, Consistency: "read-committed"}
}
...
func TestIntegrationReadInsertUpdate(t *testing.T) {
	...
	// scan starting at key should observe at least our row
...
func TestIntegrationReadKeyFrom(t *testing.T) {
	...
	node := domain.Node{ID: "n1", Role: domain.RolePrimary, Client: os.Getenv(dsnEnv)}
