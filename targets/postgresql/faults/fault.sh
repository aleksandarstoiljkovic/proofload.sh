#!/usr/bin/env bash
#
# fault.sh — inject and heal faults on a PostgreSQL cluster node for the
...
target ($TARGET) has no extra fault types.
...
do_kill() {
    case "$METHOD" in
        ...
        ssh)    sexec "sudo pkill -9 -f '$(param proc postgres)'" ;;
        *) return 3 ;;
    esac
}
do_pause() {
    case "$METHOD" in
        ...
        ssh)    sexec "sudo pkill -STOP -f '$(param proc postgres)'" ;;
        *) return 3 ;;
    esac
}
do_partition() {
...
heal() {
    case "$METHOD" in
        ...
        k8s)
            ...
            ;;
        ssh)
            sexec "sudo pkill -CONT -f '$(param proc postgres)'" 2>/dev/null || true
