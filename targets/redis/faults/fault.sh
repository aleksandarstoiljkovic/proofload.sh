#!/usr/bin/env bash
#
# fault.sh — inject and heal faults on a Redis cluster node for the
...
Killing a replica (redis2/redis3): the primary and peer keep serving; the node
rejoins and re-syncs on heal (docker heal runs `docker start`). Killing the
primary (redis1): Sentinel promotes a replica after down-after-milliseconds, but
the load driver targets a fixed endpoint and will not follow the failover.

network-partition (docker) needs --param network=<compose network>, e.g.
proofload-redis_default. Requires the matching CLI: kubectl / docker / ssh.
EOF
}
...
do_kill() {
    case "$METHOD" in
        ...
        ssh)    sexec "sudo pkill -9 -f '$(param proc redis-server)'" ;;
        *) return 3 ;;
    esac
}
do_pause() {
    case "$METHOD" in
        ...
        ssh)    sexec "sudo pkill -STOP -f '$(param proc redis-server)'" ;;
        *) return 3 ;;
    esac
}
do_partition() {
...
heal() {
    case "$METHOD" in
        ...
        k8s)
            ...
            ;;
        ssh)
            sexec "sudo pkill -CONT -f '$(param proc redis-server)'" 2>/dev/null || true
